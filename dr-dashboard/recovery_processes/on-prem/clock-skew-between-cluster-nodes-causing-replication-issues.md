...
2. **Synchronize NTP**
   ...
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- systemctl status ntpd
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- systemctl status chronyd
   
   # Restart NTP service
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- systemctl restart ntpd
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- systemctl restart chronyd
   
   # Force NTP sync
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- ntpdate -s <ntp-server>
