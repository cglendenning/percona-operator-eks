...
read -p "Enter new volume size in GB: " NEW_SIZE_GB
...
5. **Expand EBS volume if needed**
   ...
   VOLUME_ID=$(kubectl get pvc -n ${NAMESPACE} ${PVC_NAME} -o jsonpath='{.spec.volumeName}')
   ...
   kubectl exec -n ${NAMESPACE} ${POD_NAME} -- resize2fs /dev/nvme1n1
   ```
