...
3. **Add dedicated tmpdir on separate EBS volume**
   ```bash
   ...
   # On EKS, consider using a separate EBS volume with gp3 provisioned IOPS
   # for better temp file performance
   ```
...
4. **Consider separate tmpdir on dedicated EBS volume for isolation**
5. **Add monitoring for Created_tmp_disk_tables metric**
6. **Configure CloudWatch alarms for disk usage patterns (spikes followed by drops)**
