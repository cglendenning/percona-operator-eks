...
2. **Restore audit logs from backup**
   ```bash
   # Find audit log backups in S3
   aws s3 ls s3://<backup-bucket>/audit-logs/ --recursive
   
   # Download audit log backup
   aws s3 cp s3://<backup-bucket>/audit-logs/<audit-log-backup> /tmp/audit-log-restore/
