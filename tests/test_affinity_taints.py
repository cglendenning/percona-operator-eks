...
class TestAntiAffinity:
    ...
    def test_pxc_anti_affinity_rules(self, apps_v1):
        ...
        # Verify topologyKey is zone-based
    ...
    def test_proxysql_anti_affinity_rules(self, apps_v1):
        ...
        for term in required:
            ...
            assert 'zone' in topology_key.lower(), \
                f"ProxySQL anti-affinity topologyKey should contain 'zone', got: {topology_key}"
    ...
    def test_pods_distributed_across_zones(self, core_v1):
        ...
        assert len(pods.items) >= TEST_EXPECTED_NODES, \
            f"Expected at least {TEST_EXPECTED_NODES} PXC pods, found {len(pods.items)}"
        ...
        # Verify we have pods in multiple zones
        assert len(zones) >= TEST_EXPECTED_NODES, \
            f"PXC pods not distributed across enough zones: {len(zones)} zones, expected at least {TEST_EXPECTED_NODES}"
