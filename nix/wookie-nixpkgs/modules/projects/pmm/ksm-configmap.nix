# Vendored from Percona-Lab/k8s-monitoring v0.1.1 (vm-operator-k8s-stack/ksm-configmap.yaml).
{ lib, pkgs }:
let
  ...
  mkKsmConfigMap = namespace: {
    ...
    data = {
      config = builtins.readFile (yaml.generate "ksm-customresource-config.yaml" customResourceStateMetrics);
    };
  };
