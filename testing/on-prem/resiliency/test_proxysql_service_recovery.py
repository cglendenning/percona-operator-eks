...
def test_proxysql_service_recovery(core_v1, request):
    """Test that ProxySQL service endpoints recover after pod deletion"""
    console.print("[bold cyan]Starting ProxySQL service recovery test...[/bold cyan]")
    
    console.print(f"[dim]Finding ProxySQL services in namespace {TEST_NAMESPACE}...[/dim]")
    ...
    console.print(f"[cyan]Initial state check: ProxySQL service {service_name}[/cyan]")
