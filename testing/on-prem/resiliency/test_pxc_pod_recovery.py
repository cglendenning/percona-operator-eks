...
def test_pxc_pod_recovery(core_v1, request):
    """Test that PXC pods recover after being deleted"""
    console.print("[bold cyan]Starting PXC pod recovery test...[/bold cyan]")
    
    # Get initial state - verify pods are running
    console.print(f"[dim]Finding PXC pods in namespace {TEST_NAMESPACE}...[/dim]")
    ...
    console.print(f"[cyan]Initial state check: PXC pod {pod_name} is {target_pod.status.phase}[/cyan]")
