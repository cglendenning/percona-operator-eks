...
let
  ...
  alertRules = [
    ...
    }
    {
      ...
      group = "expression";
      expr = "(node_filesystem_avail_bytes{mountpoint=\"/var/lib/mysql\",node_name=~\".*db-pxc-.*\"} / node_filesystem_size_bytes{mountpoint=\"/var/lib/mysql\",node_name=~\".*db-pxc-.*\"}) * 100 < 30";
      for = "10m";
      no_data_state = "OK";
      custom_labels = {
        severity = "warning";
    ...
    }
    {
      ...
      group = "expression";
      expr = "(node_filesystem_avail_bytes{mountpoint=\"/var/lib/mysql\",node_name=~\".*db-pxc-.*\"} / node_filesystem_size_bytes{mountpoint=\"/var/lib/mysql\",node_name=~\".*db-pxc-.*\"}) * 100 < 20";
      for = "5m";
      no_data_state = "OK";
      custom_labels = {
        severity = "critical";
